...
const PlaceModal = ({ isOpen, onClose, onSave }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const data = {
        name: formData.name,
        address_id: formData.address_id ? parseInt(formData.address_id, 10) : null,
      };
      await onSave(data);
      onClose();
    } catch (err) {
      setError(err.message || 'Ошибка создания места');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="fixed inset-0 bg-black/90 flex items-center justify-center z-50" onClick={onClose}>
      <div className="bg-black border border-gray-800 w-full max-w-md" onClick={e => e.stopPropagation()}>
        <div className="flex justify-between items-center p-4 border-b border-gray-800">
          <h2 className="text-sm font-light tracking-wider">NEW PLACE</h2>
        ...
        <form onSubmit={handleSubmit} className="p-4 space-y-4">
          ...
          <div>
            <label className="block text-[10px] text-gray-500 uppercase tracking-wider mb-1">Address</label>
            <AddressSearch
              value={formData.address_id}
              onChange={(addressId) => setFormData(prev => ({ ...prev, address_id: addressId }))}
