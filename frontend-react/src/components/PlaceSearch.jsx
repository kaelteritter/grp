...
const PlaceSearch = ({ value, onChange, placeholder = "Search place..." }) => {
  ...
  const performSearch = async (searchQuery) => {
    setLoading(true);
    try {
      const res = await fetch(`http://localhost:8000/api/v1/places/?search=${encodeURIComponent(searchQuery)}&limit=10`);
        ...
        <div className="absolute z-10 w-full bg-black border border-gray-800 mt-1 max-h-60 overflow-auto">
          {results.map(place => (
            ...
            >
              ...
              {place.address && (
                <div className="text-gray-500 text-xs">{place.address.street}, {place.address.house}</div>
              )}
            </div>
          ))}
        </div>
