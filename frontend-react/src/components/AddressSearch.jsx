...
const AddressSearch = ({ value, onChange, placeholder = "Search address..." }) => {
  ...
  useEffect(() => {
    if (value) {
      fetch(`http://localhost:8000/api/v1/addresses/${value}`)
        .then(res => res.json())
        .then(data => {
          const addressStr = `${data.street}, ${data.house}${data.location ? ` (${data.location.name})` : ''}`;
          setQuery(addressStr);
        })
        .catch(console.error);
    }
  ...
  const handleSelect = (address) => {
    const displayStr = `${address.street}, ${address.house}${address.location ? ` (${address.location.name})` : ''}`;
    setQuery(displayStr);
    onChange(address.id);
    setIsOpen(false);
  };
